...
含まれる内容:
- Python規約
- 型ヒント・Pydanticモデル・命名規則
- 関数設計とエラーハンドリング
- LLM呼び出し・外部サイトアクセスの規約
- コメント規約
- セキュリティとパフォーマンス
- テストコード実装（LLM・外部依存のモック化）
- リファクタリング手法
...
含まれる内容:
- 基本原則（具体例の重要性、理由説明）
- Git運用ルール（GitHub Flow ブランチ戦略）
- コミットメッセージとPRプロセス
- テスト戦略（ピラミッドとカバレッジ、LLMモック方針）
- コードレビューのプロセス
- 品質自動化（Ruff / mypy / pytest / uv）
...
- [ ] ブランチ戦略が決まっている（個人開発フェーズはGitHub Flow推奨）
- [ ] コミットメッセージ規約が明確である
- [ ] PRテンプレートが用意されている
